...
# Function to generate a list of available booking dates excluding weekends
def generate_booking_dates(start_date, trip_duration):
    booking_dates = []
    current_date = start_date

    while trip_duration > 0:
        if current_date.weekday() < 5:  # Check if it's a weekday (0-4 for Monday to Friday)
...
def generate_dates():
    ...
    result_text.insert(tk.END, "Available Dates:\n")
...
calendar = ttk.Calendar(root, date_pattern="yyyy-mm-dd")
